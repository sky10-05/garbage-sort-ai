...
本システムでは、カメラで認識したごみに対して、選択された自治体の分別ルールを参照し、画面表示とWeb Speech APIによる音声案内を行う。
...
- 市区町村は利用開始ごとに選択し、選択した自治体IDはセッション内のみ保持する
- 選択した自治体IDは端末へ永続保存せず、画面を閉じた場合は選び直す
- 未対応ごみは判定不可として表示し、意見箱に登録できるようにする
- 条件分岐質問は最大2問までとする
- 追加情報質問は条件分岐質問とは別管理にする
- 判定履歴はDBに全件保存し、履歴画面では最新2件のみ表示する
- 画像は同意した場合のみ保存する
- 画像本体はDBに保存せず、画像パスのみ保存する
- 保存に同意しない画像は一時保存後、判定完了時に削除する
- 音声案内はブラウザのWeb Speech APIで案内文を読み上げる
...
保存に同意しない画像は一時ファイルとして扱い、判定完了後に削除する。削除に失敗した場合は利用者向けには判定結果を表示し、開発者向けログに削除失敗を記録する。

履歴画面では、`history.created_at` の降順で最新2件のみ取得して表示する。

未対応ごみ、判定不可、候補表示後も確定できなかったごみは、`feedback` に登録できるようにする。
...
- 選択した自治体IDは、DBや `localStorage` ではなくFlaskセッションで管理する
- セッション終了後は自治体IDを保持せず、次回利用時に選び直す
